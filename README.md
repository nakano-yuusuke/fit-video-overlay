...
- Chronological contact sheets and JSON metadata spanning videos and still images.
...
Generate only chronological contact sheets and their FIT-derived JSON metadata:

```bash
python fit2mp4.py --config overlay_config.json --contact-sheet-only
```

This command takes precedence over `contact_sheet.enabled`. During a normal run,
contact sheets are added only when `contact_sheet.enabled` is `true`.
...
overlays. Layout, margins, image format, and overwrite behavior are configurable.
`json_fields` maps prepared DataFrame columns to output names and can apply a
`multiplier` and `decimals`. Missing optional columns and values are omitted. Set
`processing.max_fit_duration_minutes` to `null` when covering a full ride.
...
- 動画と静止画を時系列に統合したコンタクトシートとJSONメタデータ生成
...
時系列コンタクトシートとFIT派生データJSONだけを生成できます。

```bash
python fit2mp4.py --config overlay_config.json --contact-sheet-only
```

このCLI指定は `contact_sheet.enabled` より優先されます。通常実行では
`contact_sheet.enabled=true` の場合だけ、既存出力に加えて生成します。
...
合成していない元画像を使います。`json_fields` では準備済みDataFrameの列、
出力名、`multiplier`、`decimals` を指定できます。存在しない任意列や欠損値は
JSONから省略されます。ブルベ全体を対象にする場合は
`processing.max_fit_duration_minutes` を `null` にしてください。
